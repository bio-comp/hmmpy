...
class HMM_Classifier:
    ...
    def classify(self, sample):
        ...
        if self.pos_hmm == None or self.neg_hmm == None:
            raise Exception("pos/neg hmm(s) missing")

        pos_ll = forward(self.pos_hmm, sample, scaling=True)[0]
        neg_ll = forward(self.neg_hmm, sample, scaling=True)[0]
...
def baum_welch(hmm, Obs_seqs, **args):
    ...
    for epoch in xrange(epochs):
        ...
        Expect_si_sj_all = numpy.zeros([hmm.N, hmm.N],
                                       float)  # Expectation of transitioning from state i to state j over all seqs
        ...
        for Obs in Obs_seqs:
            ...
            ow += 1
            Obs = list(Obs)
            log_Prob_Obs, Alpha, c = forward(hmm=hmm, Obs=Obs,
                                             scaling=1)  # Calculate forward probs, log-likelihood, and scaling vals
