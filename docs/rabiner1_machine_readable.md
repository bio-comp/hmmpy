...
## Machine-readable extraction package

- Source PDF: `rabiner1.pdf`
- Page count: 30
- SHA-256: `d5cb7e282741d6ace368bab6260acc5eb5a4decbc6b984a7f0f1a73c9c22517c`
- Extraction tool: PyMuPDF (fitz)
- Notes: OCR / symbol artifacts may be present; see JSON for bounding boxes and block-level structure.

## Figure index (image-informed summaries)

- **Fig. 1** (PDF page 2): 5-state Markov chain — Visible-state Markov chain example used to introduce discrete state transitions and transition probabilities before hidden states.
- **Fig. 2** (PDF page 4): One-, two-, and three-coin models — Shows multiple hidden-state model explanations for similar coin-toss observations, motivating hidden structure and model complexity tradeoffs.
- **Fig. 3** (PDF page 4): Urn-and-ball HMM analogy — Physical metaphor separating hidden state transitions (urn changes) from emissions (ball colors).
- **Fig. 4** (PDF page 6): Forward algorithm computation — Fan-in and trellis views showing how alpha values accumulate from previous states in dynamic programming.
- **Fig. 5** (PDF page 7): Backward algorithm computation — Backward message propagation (beta values) through the trellis, complementary to the forward pass.
- **Fig. 6** (PDF page 8): Computation of xi_t(i,j) — Local transition responsibility term within Baum–Welch, visually combining forward, transition/emission, and backward factors.
- **Fig. 7** (PDF page 10): HMM topologies — Compares ergodic, left-right (Bakis), and parallel-path left-right topologies; makes structural inductive bias visually explicit.
- **Fig. 8** (PDF page 12): Null-transition examples and finite-state networks — Shows silent/null arcs used to model skips, alternatives, and grammar composition without emitting observations.
- **Fig. 9** (PDF page 13): Duration modeling comparison — Contrasts standard HMM implicit durations (self-loops) with explicit variable-duration state modeling.
- **Fig. 10** (PDF page 18): Deleted interpolation state diagram — Graphical view of smoothing/interpolating between parameter sets/models using interpolation weights.
- **Fig. 11** (PDF page 19): Continuous speech recognizer block diagram — System-level architecture from feature analysis through lexical/syntactic/semantic processing with knowledge sources.
- **Fig. 12** (PDF page 20): Isolated-word HMM recognizer block diagram — Parallel likelihood scoring of word HMMs from observation sequences followed by argmax word selection.
- **Fig. 13** (PDF page 21): Front-end feature analysis pipeline — LPC/cepstral feature extraction sequence (preemphasis, framing, windowing, analysis, derivatives) before HMM decoding.
- **Fig. 14** (PDF page 22): VQ distortion vs codebook size — Diminishing returns curve showing distortion reduction as codebook size increases (log scale).
- **Fig. 15** (PDF page 22): Word error rate vs number of HMM states — Model-capacity tuning curve with a shallow best region rather than a sharply sensitive optimum.
- **Fig. 16** (PDF page 23): Estimated vs model density comparisons — Per-dimension comparisons showing where smooth parametric densities fit jagged empirical distributions poorly (motivating mixtures).
- **Fig. 17** (PDF page 23): Word error vs minimum density floor epsilon — Hyperparameter sensitivity plot showing broad stable range and degradation when epsilon becomes too small.
- **Fig. 18** (PDF page 23): Segmental k-means training flowchart — Iterative training loop visualization (initialize, segment, estimate, reestimate, converge).
- **Fig. 19** (PDF page 24): Energy / log-likelihood / state assignment for 'six' — Aligned plots linking acoustic energy and accumulated likelihood to Viterbi-like state segmentation across frames.
- **Fig. 20** (PDF page 24): State duration histograms for 'six' — Normalized duration distributions per state showing heterogeneous duration behavior across states.
- **Fig. 21** (PDF page 25): Level-building connected-digit recognizer — Architecture for connected digit recognition combining feature extraction, level-building matching, and postprocessing.
- **Fig. 22** (PDF page 26): Digit HMM characterization for connected recognition — Compact diagram of per-digit HMM components (densities, durations, energies, transitions) and inter-level interfaces.
- **Fig. 23** (PDF page 26): HMMs in the level-building algorithm — Shows geometric application of HMMs within multi-level dynamic programming for connected digits.

---

## Page-by-page extracted text (heuristic column order)
...
_Image blocks detected on page: 1_
